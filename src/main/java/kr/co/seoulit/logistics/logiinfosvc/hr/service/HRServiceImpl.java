...
public class HRServiceImpl implements HRService {
	...
	public EmpInfoTO accessToAuthority(String companyCode, String workplaceCode, String inputId, String inputPassWord)
		...
		System.out.println("고은1");
		EmpInfoTO TO = null;

		TO = checkEmpInfo(companyCode, workplaceCode, inputId);	// 데이터 베이스 에서 우리가 로그인 화면에서 입력한 값을 보내줘서 비교한후 있으면 들고와서 그사람의 정보를 bean 에 담는다
		System.out.println(TO);
		System.out.println("고은2");	

		checkPassWord(companyCode, TO.getEmpCode(), inputPassWord); // 비밀번호를 확인 해주는 메서드
		System.out.println("고은3");	
		
		
		
		String[] userAuthorityGroupList = getUserAuthorityGroupList(TO.getEmpCode()); // 사용자의 권한그룹 리스트를 가져오는 메서드
		System.out.println("고은4");
		TO.setAuthorityGroupList(userAuthorityGroupList); 
		System.out.println("고은5");
		String[] menuList = getUserAuthorityGroupMenu(TO.getEmpCode()); // 권한그룹별 메뉴 리스트를 가져오는 메서드 
		System.out.println("고은6");
		TO.setAuthorityGroupMenuList(menuList); 

		System.out.println("고은7");
