...
<html>
	 ...
	 //-----------------------------------------코드 조회------------------------------------------// 
	  ...
	  codeList.addEventListener("click", () => { 
	     ...
	     let xhr = new XMLHttpRequest();
	       xhr.open('POST', "${pageContext.request.contextPath}/compinfo/searchCodeList.do?"
	           + "method=findCodeList",
	           true);
	...
	codeDetailInsertButton.addEventListener("click", () => {
      ...
      let xhr = new XMLHttpRequest();
        xhr.open('POST', "${pageContext.request.contextPath}/compinfo/searchCodeList2.do"+
        "?method=findCodeDetailList"
          +"&divisionCodeNo="+codeDetail[0].divisionCodeNo,
            true)
        console.log("codeDetail[0].divisionCodeNo:"+codeDetail[0].divisionCodeNo);
        xhr.setRequestHeader('Accept', 'application/json');
        xhr.send();
        xhr.onreadystatechange = () => {
          if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            estDetailGridOptions.api.setRowData(txt.codeLists);
            console.log(txt.codeList);
          }
		}
	});
	...
	let newcodeRowValues=[];
	  ...
	  /*  estDetailGridOptions.api.forEachNode(function(eachRow){
	    ...
	    Swal.fire({
	      ...
	      let xhr = new XMLHttpRequest();
	      xhr.open('POST', "${pageContext.request.contextPath}/compinfo/codechange.do?method=addCodeInFormation&newCodeInfo="
