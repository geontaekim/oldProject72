...
function initEvent() {
	...
	$('#batchSaveButton').on("click" , function() {
		...
		if(resultList.length != 0 && confirmStatus) {
			...
			$.ajax({ 
				type : 'POST',
				url : '${pageContext.request.contextPath}/compinfo/batchWorkplaceListProcess.do' ,
				async :false,
				data : {
...
function showWorkplaceGrid() {
	...
	$.ajax({ 
		type : 'POST',
		url : '${pageContext.request.contextPath}/compinfo/searchWorkplace.do' ,
		async :false,
		data : {
	...
	$("#addressDialog").dialog({
		...
		open : function(event, ui) {
			...
			$('#addressSearchButton').on( "click" , function() {	
				...
				$.ajax({ 
					type : 'POST',
					url : '${pageContext.request.contextPath}/compinfo/searchAddressList.do' ,
					data : {
