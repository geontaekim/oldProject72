...
<head>
	...
	<!-- <script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=1342bc94fe690e96b3ffeb5dc5bd0304"></script>	 -->
 ...
 &nbsp;&nbsp;&nbsp;<button id="warehouseDetailInsertButton" onclick="addRow(this)">자재 추가</button>
	...
	//	const warehousePageListButton = document.querySelector("#warehousePageListButton");
		...
		warehouseSearchBtn.addEventListener("click", () => {
			...
			xhr.open('POST', '${pageContext.request.contextPath}/logiinfo/searchwarehouseList.do' +
		...
		warehouseDetailSearchBtn.addEventListener("click",()=>{
			...
			if (warehouseData[0].warehouseCode == "" || warehouseData[0].warehouseName == "") {
				...
				let xhr = new XMLHttpRequest();
				xhr.open('POST', '${pageContext.request.contextPath}/stock/searchWarehouseStockList.do' +
			...
			if(flag == 0){
				...
				if(event == "warehouse"){
					...
					Swal.fire({
						...
						if (result.isConfirmed) {
							let xhr = new XMLHttpRequest();
							xhr.open('POST', "${pageContext.request.contextPath}/logiinfo/batchwarehouseProcess.do"
				...
				//일괄 처리 : 자재
					...
					Swal.fire({
						...
						if (result.isConfirmed) {
							let xhr = new XMLHttpRequest();
							xhr.open('POST', "${pageContext.request.contextPath}/stock/batchStockProcess.do"
