...
//일반거래처 검색 조건에있는 셀렉트	
	...
	function showCustomerGrid() {
		...
		$.ajax({ 
			type : 'POST',
			url : '${pageContext.request.contextPath}/compinfo/searchCustomer.do' ,
			async :false,
			data : {
	...
	//------------------------------------------------일반거래처 일괄저장-------------------------------------------//		
		...
		if(resultList.length != 0 && confirmStatus) {
			...
			$.ajax({ 
				type : 'POST',
				url : '${pageContext.request.contextPath}/compinfo/batchCustomerListProcess.do' ,
				async :false,
				data : {
...
//금융거래처정보추가
	...
	function showFinancialAccountGrid() {
		...
		$.ajax({ 
			type : 'POST',
			url : '${pageContext.request.contextPath}/compinfo/searchFinancialAccountAssociatesList.do' ,
			async :false,
			data : {
	...
	$('#batchSaveButton2').on("click" , function() {
		...
		if(resultList.length != 0 && confirmStatus) {
			...
			$.ajax({ 
				type : 'POST',
				url : '${pageContext.request.contextPath}/compinfo/batchFinancialAccountAssociatesListProcess.do' ,
				async :false,
				data : {
					method : 'batchListProcess', 
					batchList : JSON.stringify(resultList)
