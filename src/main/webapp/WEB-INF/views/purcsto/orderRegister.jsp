...
<script>
  ...
  const orderList = () => { // 발주필요목록 가져온다 
    ...
    xhr.open('POST', '${pageContext.request.contextPath}/purchase/getOrderList.do?'
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        if (jsonData.result == "") {
          ...
          Swal.fire("오류", jsonData.errorMsg, "error");
          return;
        }
        orderListGridOptions.api.setRowData(jsonData.result);
      }
    }
  }
  ...
  orderBtn.addEventListener('click', () => {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('POST', '${pageContext.request.contextPath}/purchase/order.do?'
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        jsonData = JSON.parse(txt);
        console.log(txt);
      	if (jsonData.result.errorCode < 0) {
          swal.fire("오류", jsonData.result.errorMsg, "error");
          return;
        }
        Swal.fire("성공",jsonData.result.errorMsg, "success");
