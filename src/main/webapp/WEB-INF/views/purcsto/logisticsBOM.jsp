...
function initEvent() {
	...
	$('#batchSaveButton').on("click", function() {
		...
		if(resultList.length != 0 && confirmStatus) {
			...
			$.ajax({ 
				type : 'POST',
				url : '${pageContext.request.contextPath}/stock/batchBomListProcess.do' ,
				async :false,
				data : {
...
function showBomGrid() {
	...
	$.ajax({ 
		type : 'POST',
		url : '${pageContext.request.contextPath}/stock/searchBomDeploy.do' ,
		data : {
...
function showBomInfoGrid() {
	...
	$.ajax({ 
		type : 'POST',
		url : '${pageContext.request.contextPath}/stock/searchBomInfo.do' ,
		data : {
