...
<script>
  ...
  searchBtn.addEventListener("click", () => {
    ...
    xhr.open('POST', "${pageContext.request.contextPath}/busicon/searchEstimateInContractAvailable.do?"
        + "startDate=" + startDate
        + "&endDate=" + endDate,
        true);
  ...
  contractBatchInsertBtn.addEventListener("click", () => {
    ...
    console.log("선택된 행",newEstimateRowValues);
    ...
    if (selectedNodes[0].data.contractType == undefined) {
     ...
     Swal.fire({
      ...
      //let today = now.getFullYear() + "-" + (now.getMonth() +1 ) + "-" +  now.getDate();
      xhr.open('POST', "${pageContext.request.contextPath}/busicon/addNewContract.do?"
      ...
      xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          estDetailGridOptions.api.setRowData([]);
          // 데이터 확인
          let txt = xhr.responseText; // JSON객체가 문자열로 표시가 됨
          console.log("동기1-txt",txt);
          txt = JSON.parse(txt);
          console.log("동기2-txt",txt); //문자열의 JSON 객체를 실제 JSON 객체로 만든다.
          ...
          //Swal.fire("data",,"success")
           console.log("동기4-txt",txt.gridRowJSON);
          let conDNStr="";
          
          //지금 txt.gridRowJson을 보면 애초애 배열안에 객체가 들어가있는 형태이므로  이 코드를 써도 안먹힌다.
          const conDetailList = Object.values(txt.gridRowJSON);//어레이라이크를 배열형태로 바꿔줌
          console.log("동기5-txt",conDetailList);
