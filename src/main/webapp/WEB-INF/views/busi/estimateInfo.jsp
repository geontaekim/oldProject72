...
<div class="modal fade" id="amountModal" role="dialog">
    ...
    <script>
        ...
        const estimateDeleteBtn = document.querySelector("#estimateDeleteButton");            //견적삭제
 ...
 // ==============첫번째 그리드에 달린 버튼===============================
        ...
        estimateSearchBtn.addEventListener("click", () => { 
            ...
            xhr.open('POST','${pageContext.request.contextPath}/busi/searchEstimate.do?'
                +"startDate="+fromDate.value
                +"&endDate="+toDate.value
                +"&dateSearchCondition="+dateApply,
                true)
                console.log('${pageContext.request.contextPath}/busi/searchEstimate.do')
        ...
        estimateDeleteBtn.addEventListener("click", () => {
           ...
           console.log("selectedNodes??"+selectedNodes);
            ...
            Swal.fire({
                 ...
                 let xhr = new XMLHttpRequest();
                 xhr.open('POST',"${pageContext.request.contextPath}/busi/deleteEstimate.do"+'?estimateNo='+estimateNo,
                           true);
                 xhr.setRequestHeader('Accept', 'application/json');
                 xhr.send();
                 ...
                 xhr.onreadystatechange = () => {
                   if (xhr.readyState == 4 && xhr.status == 200) {
                     let txt = xhr.responseText;
                     console.log("txt어케되있음?"+txt);
                 
                     txt2 = JSON.parse(txt);
                      let resultMsg =
                         "<h5>< 견적 삭제 내역 ></h5>"
                         + "견적번호 : <b>"+txt2.estimateNo+"</b></br>"
                         /* txt.result.removeEstimateNo */
                         + "견적상세번호 : <b>"+txt2.estimateDetailNo+"</b></br>"
                         /* + txt.result.status  */
                         + "위와 같이 작업이 처리되었습니다"; 
                     //삭제완료 후 grid 초기화
                     estimuloInfoGridOptions.api.setRowData([]);
     ...
     // O PDF iReport  
        pdfOpenBtn.addEventListener("click", () => {
           ...
           window.open("${pageContext.request.contextPath}/logiinfo/report.html?method=estimateReport&orderDraftNo="+getRowIdValue,"window", "width=1600, height=1000")
        });
     ...
     // O EXCEL
      excelOpenBtn.addEventListener("click",()=>{
         ...
         console.log(selectedRows[0]);
           ...
           $.ajax({
              url:"${pageContext.request.contextPath}/compinfo/convertExcel.do",
              data:{
                 method:"createExcel",
   ...
   //===================================================================================
    ...
    // O 견적상세 조회 버튼   
        ...
        estimateDetaillBtn.addEventListener("click", () => {
           ...
           estimateDetailInfoGridOptions.api.setRowData([]);
            let xhr = new XMLHttpRequest();
            xhr.open('POST', '${pageContext.request.contextPath}/busi/searchEstimateDetailInfo.do' +
