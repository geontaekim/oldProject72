  ...
  completeBtn.addEventListener('click', () => { 	//검사 및 제작완료   P_WORK_SITE_COMPLETION 프로시저
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        let xhr = new XMLHttpRequest();
        xhr.open('POST', '${pageContext.request.contextPath}/quality/workCompletion.do' +
...
workSiteLogListBtn.addEventListener('click', () => {
  ...
  const workSiteLog = () => {
    ...
    console.log("_workSiteLogDate???"+_workSiteLogDate);
    if (_workSiteLogDate == "") {
      Swal.fire("알림", "조회할 날짜를 입력하십시오", "info");
      return;
    }
    let xhr = new XMLHttpRequest();
    xhr.open('POST', '${pageContext.request.contextPath}/quality/workSiteLog.do?'
