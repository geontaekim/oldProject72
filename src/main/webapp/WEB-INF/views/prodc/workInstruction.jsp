...
// 작업지시조회
  ...
  const workOrderList = () => { 
    ...
    xhr.open('POST', '${pageContext.request.contextPath}/quality/getWorkOrderableMrpList.do' +
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        console.log(txt);
        workMrpGridOptions.api.setRowData(txt.result);
      }
    }
  }
   ...
   // let mrpGatheringNo = workOrderList;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let xhr = new XMLHttpRequest();
        xhr.open('POST', '${pageContext.request.contextPath}/quality/workOrder.do?'
  ...
  const WorkOrderInfoList = () => {  // 작업지시 조회 
    let xhr = new XMLHttpRequest();
    xhr.open('POST', '${pageContext.request.contextPath}/quality/showWorkOrderInfoList.do' +
  ...
  const workOrderCompletion = () => {  
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let xhr = new XMLHttpRequest();
        xhr.open('POST', '${pageContext.request.contextPath}/quality/workOrderCompletion.do' +
