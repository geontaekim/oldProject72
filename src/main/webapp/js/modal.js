...
const getListData = (divisionCodeNo) => {
    ...
    xhr.open('POST',
        '/compinfo/codeList.do?method=findDetailCodeList&divisionCodeNo='+divisionCodeNo,
        true);
...
const getStandardUnitPrice = (itemCode, unit) => {
    ...
    // BOX
    xhr.open('POST',
        `/logiinfo/getStandardUnitPrice.do?method=getStandardUnitPrice&itemCode=${itemCode}`,
        true);
    xhr.setRequestHeader('Accept', 'application/json');
    xhr.send();
    xhr.onreadystatechange = () => {
...
const getMrpList = (mpsRowNode) => {
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            mrpGridOptions.api.updateRowData({add: jsonData.result});
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            let resultMsg =
                ...
                + (`소요량취합 일련번호 : <br>`
                + jsonData.result[0].firstMrpNo + `부터 ` + jsonData.result[0].lastMrpNo + ` 까지 <br>
                     총 ` + (jsonData.result[0].length -1) + `건 등록 완료.<br>`) + `<br>
                     위와 같이 작업이 처리되었습니다.`;
...
const registerMrpGathering = (mrpGatheringDate, mrpNoList, mrpNoAndItemCodeList) => {
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            console.log("jsonData.errorCode???"+jsonData.errorCode);
   
            let resultMsg =
                "< 소요량전개 => 소요량취합 등록 내역 ><br/>"
                + ( ( jsonData.INSERT.length != 0 ) ?
                "소요량취합 일련번호 : " + jsonData.firstMrpGatheringNo +" 부터 </br>"
                + jsonData.lastMrpGatheringNo  +" 까지 </br>"
                + jsonData.INSERT.length + " 건 등록 완료. </br>"
                + "소요량전개 일련번호 : " + jsonData.changeMrpGatheringStatus
...
const getOrderModal = (mrpGatheringList) => {
    ...
    xhr.open('POST', '/purchase/showOrderDialog.do' +
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            orderGridOptions.api.updateRowData({add: jsonData.result});
        }
    }
}
...
const getWarehousingModal = () => {
    let xhr = new XMLHttpRequest();
    xhr.open('POST', '/purchase/searchOrderInfoListOnDelivery.do' +
...
const getWorkOrderSimulationModal = (mrpGatheringNoList,mrpNoList) => {
    let xhr = new XMLHttpRequest();
    xhr.open('POST', '/quality/showWorkOrderDialog.do'
...
const getWorkSiteModal = (workSiteCourse, workOrderNo, itemClassIfication) => { // 검사항목,작업지시번호,품목분류
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('POST', '/quality/showWorkSiteSituation.do?'
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            workSiteSimulationGridOptions.api.setRowData(txt.result);
        }
    }
}
...
const getLatLng = (wareHouseCodeNo) => {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('POST',
        '/compinfo/codeList.do?method=findLatLngList&wareHouseCodeNo='+wareHouseCodeNo,
        true);
    xhr.setRequestHeader('Accept', 'application/json');
    xhr.send();
    xhr.onreadystatechange = () => {
...
const getItemImage = (itemGroupCodeNo) => {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('POST',
        '/compinfo/codeList.do?method=findDetailImageList&itemGroupCodeNo='+itemGroupCodeNo,
        true);
    xhr.setRequestHeader('Accept', 'application/json');
    xhr.send();
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            console.log("jsonData");
            console.log(jsonData);
            let imgRoute = jsonData.detailCodeList[0].itemImagelocation;
           	console.log(imgRoute);
            let www = jsonData.detailCodeList[0].itemDescription;
            if (jsonData.errorCode != 1) {
               console.log('데이터를 불러드리는데 오류가 발생했습니다.');
                swal({
                    text: '데이터를 불러드리는데 오류가 발생했습니다.',
